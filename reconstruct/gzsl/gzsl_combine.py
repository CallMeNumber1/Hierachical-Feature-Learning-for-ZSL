...
# -*- encoding: utf-8 -*-
'''
@File    :gzsl_combine.py
@Date    :2021/01/19 00:35:34
@Author  :Chong
在gzsl_multi的基础上，将fa分支与主分支合并后一起做预测。
'''
...
from hfln import Multi_HFLN,GZSL_Multi_Combine_HFLN
...
def save_model(model, optimizer, epoch, best_acc, best_epoch, fname, is_best):
    ...
    if is_best:
        shutil.copyfile(fname, './multi_models/combine_AWA2_PS_model_best.pth.tar')
    return 
if __name__ == '__main__':
    ...
    writer = SummaryWriter('visualize/gzsl/multi_combine_steplr2')

    print('--------ZSL setting-----------\n')
    print('train_data:{},test_data:{}'.format(len(train_data), len(test_data)))
    
    model = models.resnet101(pretrained=True)
    model = GZSL_Multi_Combine_HFLN(args, model, train_sig, sig, father_train_sig, father_sig)
    ...
    for ep in range(args.epochs):
        ...
        print('Testing...\n')
        model.eval()
        test_acc = gzsl_multi_evaluate(args, model, test_data, n_labels=10, origin=unseen2origin, father_n_labels=5, fa_origin=fa_unseen2origin)
        test_seen_acc = gzsl_multi_evaluate(args, model, test_seen_data, n_labels=40, origin=seen2origin, father_n_labels=8, fa_origin=fa_seen2origin)
        ...
        if is_best:
            best_epoch = ep + 1
            best_acc = HM
            save_model(model, optimizer, ep, best_acc, best_epoch, './multi_models/combine_AWA2_PS_epoch{}_HM{}_checkpoint.pth.tar'.format(best_epoch, best_acc), is_best)
