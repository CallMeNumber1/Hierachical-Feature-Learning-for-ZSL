...
'''
@File    :gzsl_multi.py
@Date    :2021/01/15 15:45:08
@Author  :Chong
在multi_main.py的基础上，增加了gzsl设置
'''
import os
import numpy as np
import torch
...
def save_model(model, optimizer, epoch, best_acc, best_epoch, fname, is_best):
    ...
    if is_best:
        shutil.copyfile(fname, './imgsize448_multi_models/steplr3_AWA2_PS_model_best.pth.tar')
    return 
if __name__ == '__main__':
    ...
    writer = SummaryWriter('visualize/gzsl/imgsize448_multi_steplr3')
    ...
    for ep in range(args.epochs):
        ...
        if is_best:
            best_epoch = ep + 1
            best_acc = HM
            save_model(model, optimizer, ep, best_acc, best_epoch, './imgsize448_multi_models/steplr3_AWA2_PS_epoch{}_HM{}_checkpoint.pth.tar'.format(best_epoch, best_acc), is_best)
